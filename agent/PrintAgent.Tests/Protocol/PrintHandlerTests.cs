...
public class PrintHandlerTests
{
    ...
    public async Task Handle_ValidParams_DelegatesToServiceAndReturnsJobId()
    {
        ...
        jobs.SubmitAsync(Arg.Any<string>(), Arg.Any<byte[]>(), Arg.Any<PrintOptions>(),
            ...
            .Returns(jobId);
        var handler = new PrintHandler(jobs, maxBytes: 20 * 1024 * 1024);
    ...
    {
        var handler = new PrintHandler(Substitute.For<IPrintJobSubmitter>(), maxBytes: 1024);
    ...
    {
        var handler = new PrintHandler(Substitute.For<IPrintJobSubmitter>(), maxBytes: 100);
